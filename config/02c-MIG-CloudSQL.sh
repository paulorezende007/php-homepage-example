...
NETWORK="vpc-main-01"
SUBNET="subnet-01"
STARTUP_SCRIPT_URL="gs://bucket-dgc-arquitetura-tradicional/startup-script-mig.sh"
...
# Grupo de Instâncias
 gcloud beta compute health-checks create tcp $HEALTHCHECK_NAME --project=$PROJECT_ID --port=80 --proxy-header=NONE \
  --no-enable-logging --check-interval=30 --timeout=20 --unhealthy-threshold=3 --healthy-threshold=3 
 ...
 gcloud beta compute instance-groups managed set-autoscaling $INSTANCE_GROUP_NAME --project=$PROJECT_ID --region=$REGION\
  --cool-down-period=120 --max-num-replicas=6 --min-num-replicas=2 --mode=on --target-cpu-utilization=0.6
